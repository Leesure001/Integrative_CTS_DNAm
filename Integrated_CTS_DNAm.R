...
# 2. Check Spearman correlations for estimated cell type proportions
check_frac_cor <- function(frac_1, frac_2, method){
    library(assertthat)
    assert_that(all(colnames(frac_1) == colnames(frac_2)), msg = "Cell types in two data are not in the same order.")
    assert_that(all(rownames(frac_1) == rownames(frac_2)), msg = "Samples in two data are not in the same order.")
    frac_cor <- c()
    for (i in seq_along(colnames(frac_1))){
        frac_cor <- c(frac_cor, cor(frac_1[, i], frac_2[, i], method = method))
    }
    frac_cor <- matrix(frac_cor, ncol = 1)
    rownames(frac_cor)  <- colnames(frac_1)
    colnames(frac_cor)  <- paste0(method, "_cor")
    return (frac_cor)
}

# 3. Modified Version For HIRE (initialized using inputting cell type proportions)
...
# 6. Extract differentially methylated CpGs from DMCT list (The cell types in this function need to be modified case by case.)
...
# 7. Generate coe list for TCA (The cell types in this function need to be modified case by case.)
...
# 8. Generate coe list for TOAST (The cell types in this function need to be modified case by case.)
...
# 9. Generate coe list for CeDAR (The cell types in this function need to be modified case by case.)
...
# 10. Generate coe list for HIRE V2 (The cell types in this function need to be modified case by case.)
...
# 11. Integrated analysis with averaging p values - V1 (The cell types in this function need to be modified case by case.)
...
# 12. Integrated analysis with minimizing p values - V2 (The cell types in this function need to be modified case by case.)
...
# 13. Generate UpSet Plot (The cell types in this function need to be modified case by case.)
